...
function isDistraction(appName) {
    ...
    const customDistractions = store.get('settings.customDistractions', []);
...
// Activity logging
function logActivity(appName, windowTitle) {
...
// Focus sessions
function startFocusSession(type, durationMinutes) {
    const sessions = store.get('focusSessions', []);
    const nextIds = store.get('nextIds');
    const session = {
        id: nextIds.session,
...
function endFocusSession(id, qualityScore, distractions) {
    const sessions = store.get('focusSessions', []);
...
function setupIpcHandlers() {
    ...
    ipcMain.handle('set-setting', (_e, key, value) => { store.set(`settings.${key}`, value); return true; });
    ipcMain.handle('window-minimize', () => mainWindow?.minimize());
    ipcMain.handle('window-maximize', () => mainWindow?.isMaximized() ? mainWindow.unmaximize() : mainWindow?.maximize());
    ipcMain.handle('window-close', () => mainWindow?.hide());
    ipcMain.handle('get-tracking-status', () => isTracking);
    ipcMain.handle('set-tracking-status', (_e, status) => { status && !isTracking ? startTracking() : !status && isTracking ? stopTracking() : null; return isTracking; });
    ...
    ipcMain.handle('start-focus-session', (_e, type, duration) => startFocusSession(type, duration));
    ipcMain.handle('end-focus-session', (_e, id, quality, distractions) => endFocusSession(id, quality, distractions));
    ipcMain.handle('get-tasks', () => getTasks());
    ipcMain.handle('create-task', (_e, title, priority) => createTask(title, priority));
    ipcMain.handle('update-task-status', (_e, id, status) => updateTaskStatus(id, status));
    ipcMain.handle('delete-task', (_e, id) => deleteTask(id));
    // Phase 5: Knowledge Management handlers
    ipcMain.handle('get-notes', () => store.get('notes', []));
    ipcMain.handle('save-notes', (_e, notes) => { store.set('notes', notes); return true; });
    ...
    ipcMain.handle('save-flashcards', (_e, cards) => { store.set('flashcards', cards); return true; });
    // Phase 5: Agent handlers
    ipcMain.handle('get-agent-tasks', () => store.get('agentTasks', []));
    ipcMain.handle('save-agent-tasks', (_e, tasks) => { store.set('agentTasks', tasks); return true; });
}
app.whenReady().then(() => {
    ...
    registerShortcuts();
    setupIpcHandlers();
    if (store.get('settings.autoStartTracking'))
        startTracking();
