...
export default function FocusPage() {
    ...
    return (<div className="max-w-2xl mx-auto py-8">
            ...
            <div className="flex justify-center mb-8">
                <div className="relative w-72 h-72">
                    <svg className="w-full h-full -rotate-90">
                        ...
                        <circle cx="144" cy="144" r="130" fill="none" stroke={timer.mode === 'focus' ? '#14b8a6' : '#f59e0b'} strokeWidth="12" strokeDasharray={2 * Math.PI * 130} strokeDashoffset={2 * Math.PI * 130 * (1 - progress / 100)} strokeLinecap="round" className="transition-all duration-1000"/>
                    </svg>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        ...
                        {timer.isRunning && timer.distractionCount > 0 && (<div className="text-red-400 text-sm mt-2">
            ...
            <div className="flex justify-center gap-4 mb-8">
                <button onClick={timer.isRunning ? pauseTimer : startTimer} className={`w-16 h-16 rounded-full flex items-center justify-center transition-all ${timer.isRunning
            ? 'bg-primary-500/20 text-primary-400 hover:bg-primary-500/30'
            : 'bg-primary-500 text-white hover:bg-primary-600'}`}>
                    {timer.isRunning ? <Pause className="w-7 h-7"/> : <Play className="w-7 h-7 ml-1"/>}
                </button>

                <button onClick={resetTimer} className="w-14 h-14 rounded-full bg-dark-700 text-dark-400 hover:bg-dark-600 flex items-center justify-center transition-all">
                    <RotateCcw className="w-6 h-6"/>
                </button>

                <button onClick={() => setSoundEnabled(!soundEnabled)} className="w-14 h-14 rounded-full bg-dark-700 text-dark-400 hover:bg-dark-600 flex items-center justify-center transition-all">
            ...
            <div className="flex justify-center gap-3 mb-8">
                {SESSION_PRESETS.map((p, i) => (<button key={i} onClick={() => selectPreset(i)} disabled={timer.isRunning} className={`px-6 py-3 rounded-xl transition-all ${selectedPreset === i
            ...
            <div className="flex justify-center gap-2 mb-8">
                <button onClick={() => !timer.isRunning && setTimer(prev => ({
            ...prev,
            mode: 'focus',
            timeRemaining: preset.focus * 60,
        }))} disabled={timer.isRunning} className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-all ${timer.mode === 'focus'
        ...
        }))} disabled={timer.isRunning} className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-all ${timer.mode === 'break'
