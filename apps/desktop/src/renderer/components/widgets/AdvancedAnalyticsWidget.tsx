...
export default function AdvancedAnalyticsWidget() {
    ...
    const maxFocusTime = Math.max(...weeklyTrends.map(t => t.focusTime));
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-1 bg-dark-900 rounded-lg p-1">
                    <button
                        ...
                        className={`px-3 py-1 text-xs rounded ${activeTab === 'trends' ? 'bg-primary-500 text-white' : 'text-dark-400 hover:text-white'}`}
                    >
                        Trends
                    </button>
                    <button
                        onClick={() => setActiveTab('breakdown')}
                        className={`px-3 py-1 text-xs rounded ${activeTab === 'breakdown' ? 'bg-primary-500 text-white' : 'text-dark-400 hover:text-white'}`}
                    >
                        Breakdown
                    </button>
                    <button
                        onClick={() => setActiveTab('insights')}
                        className={`px-3 py-1 text-xs rounded ${activeTab === 'insights' ? 'bg-primary-500 text-white' : 'text-dark-400 hover:text-white'}`}
                    >
                        Insights
                    </button>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="bg-dark-900 rounded-lg p-3">
                            ...
                            <div className="text-xl font-bold text-white mt-1">6h 42m</div>
                        </div>
                    </div>
                    ...
                    <div className="h-36">
                        <div className="flex items-end justify-between h-28 gap-1">
                            {weeklyTrends.map((trend, idx) => {
                                ...
                                return (
                                    <div key={trend.day} className="flex-1 flex flex-col items-center gap-1 group">
                                        <div className="w-full flex flex-col justify-end h-24 relative">
                                            <div className="absolute -top-6 left-1/2 -translate-x-1/2 px-1.5 py-0.5 bg-dark-700 rounded text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10">
                                        ...
                                        <span className={`text-[10px] ${isToday ? 'text-primary-400 font-semibold' : 'text-dark-500'}`}>
                                            {trend.day}
                                        </span>
                                    </div>
                                );
                        ...
                        <div className="flex-1 space-y-2">
                            {categoryBreakdown.map(cat => (
                                <div key={cat.category} className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-dark-400 flex-1">{cat.category}</span>
                                    <span className="text-xs text-white">{cat.percentage}%</span>
                                </div>
                            ))}
                        </div>
                    </div>
