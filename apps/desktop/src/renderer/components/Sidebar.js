...
import { LayoutDashboard, Timer, CheckSquare, FolderKanban, BarChart3, Settings, Play, Pause, TrendingUp, Code, Plug, Brain, BookOpen, Bot, GraduationCap, Globe, Cloud, Trophy } from 'lucide-react';
const navItems = [
...
export default function Sidebar({ isTracking, onTrackingToggle }) {
    ...
    return (<aside className="w-64 bg-dark-950 border-r border-dark-800 flex flex-col">
            {/* Tracking status */}
            <div className="p-4 border-b border-dark-800">
                <button onClick={onTrackingToggle} className={`w-full flex items-center justify-center gap-2 py-3 px-4 rounded-lg font-medium transition-all duration-200 ${isTracking
            ...
            : 'bg-dark-700 text-dark-300 hover:bg-dark-600'}`}>
                ...
                {/* Current activity */}
                {isTracking && currentActivity && (<div className="mt-3 p-2 bg-dark-800 rounded-lg">
            ...
            <nav className="flex-1 p-4 space-y-1">
                {navItems.map(({ path, icon: Icon, label }) => (<NavLink key={path} to={path} className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${isActive
