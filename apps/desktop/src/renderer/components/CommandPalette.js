...
import { Command, Search, Timer, BarChart3, Brain, Settings, Moon, Sun, Cloud, Zap, X, ChevronRight, Keyboard, Plus, PlayCircle } from 'lucide-react';
export default function CommandPalette({ isOpen, onClose, darkMode, onDarkModeToggle }) {
    ...
    return (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-start justify-center pt-20">
            <div className="w-full max-w-xl bg-dark-800 rounded-xl border border-dark-700 shadow-2xl overflow-hidden">
                ...
                <div className="flex items-center gap-3 px-4 py-3 border-b border-dark-700">
                    ...
                    <input type="text" autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Type a command or search..." className="flex-1 bg-transparent text-white placeholder-dark-400 outline-none"/>
                ...
                <div className="max-h-96 overflow-auto">
                    {/* Actions */}
                    {actionActions.length > 0 && (<div className="p-2">
                            <div className="px-2 py-1 text-xs font-medium text-dark-500 uppercase">Actions</div>
                            {actionActions.map(action => (<ActionItem key={action.id} action={action}/>))}
                        </div>)}

                    {/* Navigation */}
                    {navigationActions.length > 0 && (<div className="p-2 border-t border-dark-700">
                            <div className="px-2 py-1 text-xs font-medium text-dark-500 uppercase">Navigation</div>
                            {navigationActions.map(action => (<ActionItem key={action.id} action={action}/>))}
                        </div>)}

                    {/* Settings */}
                    {settingActions.length > 0 && (<div className="p-2 border-t border-dark-700">
                            <div className="px-2 py-1 text-xs font-medium text-dark-500 uppercase">Settings</div>
                            {settingActions.map(action => (<ActionItem key={action.id} action={action}/>))}
                        </div>)}

                    {/* No Results */}
                    {filteredActions.length === 0 && (<div className="p-8 text-center text-dark-400">
...
function ActionItem({ action }) {
    ...
    return (<button onClick={action.action} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left hover:bg-dark-700 group">
            <Icon className="w-4 h-4 text-dark-400 group-hover:text-primary-400"/>
            <div className="flex-1">
                <div className="text-white">{action.label}</div>
                {action.description && (<div className="text-xs text-dark-500">{action.description}</div>)}
            </div>
            {action.shortcut && (<kbd className="px-2 py-0.5 text-xs bg-dark-700 rounded text-dark-400">
                    {action.shortcut}
                </kbd>)}
