...
let researchHistory: ResearchSession[] = [];
let nextSessionId = 1;
 ...
 */
async function generateFallbackResults(query: string): Promise<SearchResult[]> {
    try {
...
---`;
        ...
        const response = await queryWithFallback(
            prompt,
            'You are a research assistant. Generate realistic, helpful search result summaries.',
            500
        );
 ...
 */
export async function conductResearch(query: string): Promise<ResearchSession> {
    // Search the web
...
- [finding 3]`;
    ...
    try {
        const response = await queryWithFallback(
            prompt,
            'You are a research analyst. Synthesize information from multiple sources into clear summaries.',
            800
        );
...
export function getResearchHistory(): ResearchSession[] {
    return [...researchHistory];
}

export function getResearchSession(id: number): ResearchSession | undefined {
    return researchHistory.find(s => s.id === id);
}

export function deleteResearchSession(id: number): void {
...
export function updateResearchConfig(newConfig: Partial<ResearchConfig>): void {
...
export function markAsSavedToKnowledge(sessionId: number): void {
 ...
 */
export function generateNoteFromSession(session: ResearchSession): {
    title: string;
    content: string;
    tags: string[];
} {
...
Provide a direct, helpful answer in 2-3 sentences.`;
    ...
    try {
        const answer = await queryWithFallback(
            prompt,
