...
export interface FocusPrediction {
    hour: number;
    dayOfWeek: number;
    score: number; // 0-100, higher = better for focus
...
let predictions: Map<string, FocusPrediction> = new Map();
...
export async function loadHistoricalData(): Promise<void> {
    try {
        ...
        const saved = settings.focusHistory as HistoricalData[] | undefined;
...
function calculatePredictions(): void {
    ...
    const hourDayData: Map<string, { total: number; count: number; quality: number }> = new Map();
        ...
        for (let hour = 0; hour < 24; hour++) {
            ...
            if (data && data.count > 0) {
                ...
                score = Math.min(100, Math.max(0,
                    (avgQuality * 0.6) + (Math.min(avgMinutes / 60, 1) * 40)
                ));
...
export function getPrediction(day: number, hour: number): FocusPrediction {
    const key = `${day}-${hour}`;
...
export function getBestFocusTimes(count: number = 5): FocusPrediction[] {
...
export function getCurrentPrediction(): FocusPrediction {
    const now = new Date();
    return getPrediction(now.getDay(), now.getHours());
}

export function getNextBestFocusTime(): { hour: number; day: string; score: number } | null {
    const now = new Date();
    ...
    // Look ahead 48 hours
    let best: FocusPrediction | null = null;
...
export function analyzeFocusPatterns(): FocusPattern {
    const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    ...
    // Find best hours
    const hourAvgs = hourScores.map((score, i) =>
    ...
    const dayAvgs = dayScores.map((score, i) =>
